...
V2 uses database views forward compatibility (e.g., `v2_policies`, `v2_reports`, `v2_test_results`). Models like `V2::Report` are view-backed, while `V2::Policy` and models related to imported OpenSCAP content are backed by a real table. The `V2::System` model aliases `inventory.hosts` (read-only in production).
...
No AI model should in any case generate migrations. This task is potentially dangerous and should always be done by a human developer.
...
podman-compose exec rails bundle exec rake test:validate
...
- `InventoryViews` - Inventory Views
...
- RSpec for API spec generation tests (spec/integration/)
...
Any non-necessarily literal strings should be generated using the `Faker` gem.
...
To speed up development, a compose file (`docker-compose.yml`) is also used to quickly set up the project.
All the commands should be ran inside the container context.
