...
RSpec.describe KesselBuilder, type: :service do
  ...
  describe '.build_oauth_credentials' do
    ...
    context 'when OIDC discovery fails' do
      before do
        allow(described_class).to receive(:fetch_oidc_discovery).and_raise(StandardError, 'Discovery failed')
        allow(Rails.logger).to receive(:error)
      end

      it 'raises ConfigurationError and logs error' do
        expect do
          described_class.build_oauth_credentials
        end.to raise_error(KesselBuilder::ConfigurationError,
                           /Failed to build OAuth credentials/)
        expect(Rails.logger).to have_received(:error).with(/Failed to build OAuth credentials/)
      end
    end
  end
