...
RSpec.describe Xccdf::SecurityGuides do
  BenchmarkParser = Struct.new(:id, :version, :title, :description, :profiles, :rules, keyword_init: true)

  DummySupported = Struct.new(:version, :package, keyword_init: true)
  ...
  subject(:service) do
    Class.new do
      ...
      def security_guide
        super.tap do |guide|
          guide.os_major_version ||= @os_major_version
        end
      end
  ...
  let(:supported_entry) { DummySupported.new(version: op_security_guide.version, package: 'ssg-rhel7') }
  ...
  describe '#save_security_guide' do
    ...
    it 'does not touch an already up-to-date record' do
      existing = create(:v2_security_guide,
  ...
  describe '#security_guide_contents_equal_to_op?' do
    ...
    let(:profiles_relation) { guide_record.profiles }
    ...
    before do
      ...
      allow(guide_record).to receive(:profiles).and_return(profiles_relation)
      allow(profiles_relation).to receive(:canonical).and_return(profiles_relation)
    end

    context 'when profile and rule counts match the parser data' do
      before do
        create_list(:v2_profile, op_security_guide.profiles.count, security_guide: guide_record)
        create_list(:v2_rule, op_security_guide.rules.count, security_guide: guide_record)
      end
    ...
    context 'when the stored data diverges from the parser feed' do
      before do
        create_list(:v2_profile, 1, security_guide: guide_record)
        create_list(:v2_rule, op_security_guide.rules.count, security_guide: guide_record)
      end
