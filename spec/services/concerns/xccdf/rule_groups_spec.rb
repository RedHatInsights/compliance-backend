...
RSpec.describe Xccdf::RuleGroups do
  ...
  let!(:parent_rg) do
    FactoryBot.create(:v2_rule_group,
      security_guide: security_guide,
  ...
  describe '#save_rule_groups' do
    it 'upserts the rule groups' do
      ...
      expect(parent_rg.reload.attributes.slice('title', 'description', 'rationale', 'precedence')).to eq(
      ...
      expect(created.attributes.slice('title', 'description', 'rationale', 'precedence')).to eq(
    ...
    context 'having a preceding rule group' do
      ...
      it 'changes the precedence of rule groups when needed' do
        expect {
          service.save_rule_groups
        }.to change(parent_rg, :precedence).to(op_rule_groups.index(op_parent_rg)).and
        change(preceding_rg, :precedence).to(op_rule_groups.index(op_preceding_rg))
      end
    end
    ...
    context 'when the import runs multiple times with the same data' do
      it 'does not create duplicate rule groups' do
        expect do
          service.save_rule_groups
        end.not_to change(V2::RuleGroup.where(security_guide: security_guide), :count)
      end

      it 'keeps existing ancestry relationships intact' do
        child_group = V2::RuleGroup.find_by(ref_id: child_parser.id, security_guide: security_guide)

        expect do
          service.save_rule_groups
        end.not_to change(child_group, :ancestry)
      end
    end
  end
end
