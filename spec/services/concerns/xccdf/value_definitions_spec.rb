...
RSpec.describe Xccdf::ValueDefinitions do
  ...
  let(:op_value_definitions) { [updated_op_value, new_op_value] }

  let!(:existing_definition) do
    create(:v2_value_definition,
           security_guide: security_guide,
           ref_id: updated_op_value.id,
           description: 'stale description',
           default_value: 'permissive')
  end

  let!(:unrelated_definition) { create(:v2_value_definition) }
  ...
  describe '#save_value_definitions' do
    ...
    it 'updates existing definitions with the parser attributes' do
      expect(existing_definition.reload.description).to eq(updated_op_value.description)
      expect(existing_definition.default_value).to eq(updated_op_value.value)
    end
    ...
    it 'leaves value definitions that belong to other guides untouched' do
      expect { unrelated_definition.reload }.not_to raise_error
    end
  end
end
