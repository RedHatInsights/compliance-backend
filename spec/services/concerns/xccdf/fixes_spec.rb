...
RSpec.describe Xccdf::Fixes do
  ...
  describe '#save_fixes' do
    let(:refreshed_fix) { V2::Fix.find_by(rule: rule_with_existing_fix, system: updated_op_fix.system) }
    ...
    it 'upserts parsed fixes' do
      ...
      expect(created_fix.attributes.slice('strategy', 'disruption', 'complexity', 'text')).to eq(
      ...
      expect(refreshed_fix.attributes.slice('strategy', 'disruption', 'complexity', 'text')).to eq(
        'strategy' => updated_op_fix.strategy,
        'disruption' => updated_op_fix.disruption,
        'complexity' => updated_op_fix.complexity,
        'text' => updated_op_fix.text
      )
    end
  end
  ...
  describe '#fixes' do
    it 'returns all the parsed fixes that were changed' do
      ...
      expect(refreshed_fix.system).to eq(updated_op_fix.system)
      expect(refreshed_fix.attributes.slice('strategy', 'disruption', 'complexity', 'text')).to eq(
        'strategy' => updated_op_fix.strategy,
        'disruption' => updated_op_fix.disruption,
        'complexity' => updated_op_fix.complexity,
      ...
      expect(created_fix.rule_id).to eq(rule_with_new_fix.id)
      expect(created_fix.system).to eq(new_op_fix.system)
      expect(created_fix.attributes.slice('strategy', 'disruption', 'complexity', 'text')).to eq(
        'strategy' => new_op_fix.strategy,
        'disruption' => new_op_fix.disruption,
        'complexity' => new_op_fix.complexity,
