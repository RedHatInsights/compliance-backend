...
describe 'Tailorings', swagger_doc: 'v2/openapi.json' do
  ...
  path '/policies/{policy_id}/tailorings' do
    ...
    get 'Request Tailorings' do
      ...
      ids_only_param
      sort_params_v2(Tailoring)
      search_params_v2(Tailoring)
      ...
      response '200', 'Lists Tailorings' do
        let(:version) { Tailoring.first.os_minor_version }
    ...
    post 'Create a Tailoring' do
      let(:policy_id) do
        FactoryBot.create(
          :policy,
          account: user.account,
          profile: FactoryBot.create(:profile, ref_id_suffix: 'foo', supports_minors: [1])
        ).id
      end
  ...
  path '/policies/{policy_id}/tailorings/{tailoring_id}' do
    ...
    let(:item) do
      FactoryBot.create(:tailoring, policy: Policy.find(policy_id), os_minor_version: 1)
    end
    ...
    get 'Request a Tailoring' do
      ...
      response '200', 'Returns a Tailoring' do
        let(:tailoring_id) { item.os_minor_version }
        v2_item_schema('tailoring')

        after { |e| autogenerate_examples(e, 'Returns a Tailoring when using os_minor_version') }
    ...
    patch 'Update a Tailoring' do
      ...
      let(:data) do
        {
          value_overrides: { FactoryBot.create(:value_definition, security_guide: item.security_guide).id => '123' }
        }
      end
      ...
      response '202', 'Updates a Tailoring' do
        ...
        after { |e| autogenerate_examples(e, 'Returns the updated Tailoring when using os_minor_version') }
  ...
  path '/policies/{policy_id}/tailorings/{tailoring_id}/rule_tree' do
    let(:policy_id) do
      FactoryBot.create(
        :policy,
        account: user.account,
        profile: FactoryBot.create(:profile, rule_count: 10, ref_id_suffix: 'foo', supports_minors: [1])
      ).id
    end

    let(:item) do
      FactoryBot.create(:tailoring, :with_mixed_rules, policy: Policy.find(policy_id), os_minor_version: 1)
    end
  ...
  path '/policies/{policy_id}/tailorings/{tailoring_id}/tailoring_file.json' do
    let(:policy_id) do
      FactoryBot.create(:policy, :for_tailoring, account: user.account, supports_minors: [1]).id
    end
    ...
    get 'Request a Tailoring file' do
      ...
      response '200', 'Returns a Tailoring File' do
        ...
        after { |e| autogenerate_examples(e, 'Returns a Tailoring File while using os_minor_version') }
