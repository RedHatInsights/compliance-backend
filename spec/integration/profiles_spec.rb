...
describe 'Profiles', swagger_doc: 'v2/openapi.json' do
  ...
  path '/security_guides/{security_guide_id}/profiles' do
    before { FactoryBot.create_list(:profile, 25, security_guide_id: security_guide_id) }
    ...
    get 'Request Profiles' do
      ...
      ids_only_param
      sort_params_v2(Profile)
      search_params_v2(Profile)
      ...
      response '200', 'Lists Profiles' do
        let(:filter) { "(title=\"#{Profile.first.title}\")" }
        v2_collection_schema 'profile'

        after { |e| autogenerate_examples(e, "List of Profiles filtered by '(title=#{Profile.first.title})'") }
  ...
  path '/security_guides/{security_guide_id}/profiles/{profile_id}' do
    let(:item) { FactoryBot.create(:profile) }
  ...
  path '/security_guides/{security_guide_id}/profiles/{profile_id}/rule_tree' do
    let(:item) { FactoryBot.create(:profile, rule_count: 10) }
