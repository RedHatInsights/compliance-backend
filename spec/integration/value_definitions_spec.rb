...
describe 'Value Definitions', swagger_doc: 'v2/openapi.json' do
  let(:'X-RH-IDENTITY') { FactoryBot.create(:user).account.identity_header.raw }
  ...
  path '/security_guides/{security_guide_id}/value_definitions' do
    before { FactoryBot.create_list(:value_definition, 25, security_guide_id: security_guide_id) }
    ...
    get 'Request Value Definitions' do
      ...
      ids_only_param
      sort_params_v2(ValueDefinition)
      search_params_v2(ValueDefinition)
      ...
      response '200', 'Lists Value Definitions' do
        let(:filter) { "(title=\"#{ValueDefinition.first.title}\")" }
        v2_collection_schema 'value_definition'

        after do |e|
          autogenerate_examples(e, "List of Value Definitions filtered by '(title=#{ValueDefinition.first.title})'")
        end
  ...
  path '/security_guides/{security_guide_id}/value_definitions/{value_definition_id}' do
    let(:item) { FactoryBot.create(:value_definition) }
