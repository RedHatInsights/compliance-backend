...
describe 'Security Guides', swagger_doc: 'v2/openapi.json' do
  ...
  path '/security_guides' do
    before { FactoryBot.create_list(:security_guide, 25) }
    ...
    get 'Request Security Guides' do
      ...
      ids_only_param
      sort_params_v2(SecurityGuide)
      search_params_v2(SecurityGuide)
  ...
  path '/security_guides/{security_guide_id}' do
    let(:item) { FactoryBot.create(:security_guide) }
  ...
  path '/security_guides/{security_guide_id}/rule_tree' do
    let(:item) { FactoryBot.create(:security_guide, rule_count: 10) }
