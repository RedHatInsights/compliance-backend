...
describe 'Tailorings', swagger_doc: 'v2/openapi.json' do
  ...
  path '/policies/{policy_id}/tailorings/{tailoring_id}/rules/{id}' do
    let(:canonical_profile) { FactoryBot.create(:v2_profile, rule_count: 3, supports_minors: [1]) }
    ...
    patch 'Assign a Rule to a Tailoring' do
      let(:item) { FactoryBot.create(:v2_rule, security_guide: canonical_profile.security_guide) }
    ...
    delete 'Unassign a Rule from a Tailoring' do
      let(:item) { V2::Tailoring.find(tailoring_id).rules.first }
