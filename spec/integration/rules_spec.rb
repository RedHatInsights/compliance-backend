...
describe 'Rules', swagger_doc: 'v2/openapi.json' do
  ...
  path '/security_guides/{security_guide_id}/rules' do
    before { FactoryBot.create_list(:rule, 25, security_guide_id: security_guide_id) }

    let(:security_guide_id) { FactoryBot.create(:security_guide).id }
  ...
  path '/security_guides/{security_guide_id}/profiles/{profile_id}/rules' do
    before { FactoryBot.create_list(:rule, 25, security_guide_id: security_guide_id, profile_id: profile_id) }

    let(:security_guide_id) { FactoryBot.create(:security_guide).id }
    let(:profile_id) { FactoryBot.create(:profile, security_guide_id: security_guide_id).id }
  ...
  path '/security_guides/{security_guide_id}/profiles/{profile_id}/rules/{rule_id}' do
    let(:item) { FactoryBot.create(:rule, profile_id: profile_id) }
    ...
    get 'Request a Rule assigned to a Profile' do
      ...
      response '200', 'Returns a Rule assigned to a Profile' do
        let(:security_guide_id) { Profile.find(profile_id).security_guide_id }
        let(:profile_id) { FactoryBot.create(:profile).id }
        let(:rule_id) { item.id }
  ...
  path '/policies/{policy_id}/tailorings/{tailoring_id}/rules' do
    before do
      FactoryBot.create(
        :rule,
        security_guide: Tailoring.find(tailoring_id).profile.security_guide,
    ...
    let(:policy_id) { FactoryBot.create(:policy, account: user.account, supports_minors: [9]).id }
    ...
    post 'Bulk assign Rules to a Tailoring' do
      let(:items) do
        FactoryBot.create_list(:rule, 25, security_guide: Tailoring.find(tailoring_id).security_guide)
      end
  ...
  path '/policies/{policy_id}/tailorings/{tailoring_id}/rules/{rule_id}' do
    ...
    patch 'Assign a Rule to a Tailoring' do
      let(:item) do
        FactoryBot.create(:rule, security_guide: Tailoring.find(tailoring_id).profile.security_guide)
      end
