...
RSpec.describe 'cleanup_db task' do
  ...
  describe 'cleanup_db' do
    it 'removes dangling records' do
      policy = create(:v2_policy, supports_minors: [0], system_count: 1)
      tailoring = policy.tailorings.first
      rule = create(:v2_rule, security_guide: policy.profile.security_guide)

      create(:v2_account, org_id: '8675309')
      ...
      expect do
        suppress_stdout { Rake::Task['cleanup_db'].execute }
         ...
         .and change(V2::TestResult, :count).by(-1)
         .and change(V2::RuleResult, :count).by(-1)
         .and change(V2::PolicySystem, :count).by(-1)
    end
