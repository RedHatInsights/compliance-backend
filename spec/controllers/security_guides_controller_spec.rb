...
describe SecurityGuidesController do
  let(:attributes) do
    {
      ref_id: :ref_id,
      title: :title,
  ...
  describe 'GET index' do
    let(:extra_params) { {} }
    let(:item_count) { 2 }
    let(:items) { FactoryBot.create_list(:security_guide, item_count).sort_by(&:id) }
  ...
  describe 'GET rule_tree' do
    let(:item) { FactoryBot.create(:security_guide, rule_count: 5) }
