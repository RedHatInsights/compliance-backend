...
describe TailoringsController do
  let(:attributes) do
    {
  ...
  context '/policies/:id/tailorings' do
    describe 'GET index' do
      let(:canonical_profiles) do
        item_count.times.map do |version|
          FactoryBot.create(:profile, ref_id_suffix: 'foo', supports_minors: [version])
        end
      end

      let(:parent) { FactoryBot.create(:policy, account: current_user.account, profile: canonical_profiles.last) }
      let(:extra_params) { { ref_id: pw(parent.ref_id), policy_id: parent.id } }
      let(:item_count) { 3 }

      let(:items) do
        item_count.times.map do |version|
          FactoryBot.create(:tailoring, policy: parent, os_minor_version: version)
        end.sort_by(&:id)
      end
    ...
    describe 'GET show' do
      ...
      let(:parent) { FactoryBot.create(:policy, account: current_user.account, profile: canonical_profile) }
      let(:item) { FactoryBot.create(:tailoring, policy: parent, os_minor_version: os_minor_version) }
      let(:extra_params) { { policy_id: parent.id, id: item.id } }

      let(:canonical_profile) do
        FactoryBot.create(:profile, ref_id_suffix: 'foo', supports_minors: [os_minor_version])
      end
    ...
    describe 'GET rule_tree' do
      ...
      let(:item) do
        FactoryBot.create(:tailoring, :with_mixed_rules, policy: parent, os_minor_version: os_minor_version)
      end

      let(:canonical_profile) do
        FactoryBot.create(:profile, rule_count: 5, ref_id_suffix: 'foo', supports_minors: [os_minor_version])
      end
    ...
    describe 'POST create' do
      ...
      context 'tailoring already exists' do
        before { FactoryBot.create(:tailoring, policy: parent, os_minor_version: os_minor_version) }
      ...
      context 'unsupported OS minor version' do
        let(:canonical_profile) { FactoryBot.create(:profile, ref_id_suffix: 'foo', supports_minors: []) }
    ...
    describe 'PATCH update' do
      ...
      context 'string value for a string value definition' do
        let(:value_definition) do
          FactoryBot.create(:value_definition, value_type: 'string', security_guide: item.security_guide)
        end
      ...
      context 'non-number value for a numeric value definition' do
        let(:value_definition) do
          FactoryBot.create(:value_definition, value_type: 'number', security_guide: item.security_guide)
        end
      ...
      context 'non-boolean value for a boolean value definition' do
        let(:value_definition) do
          FactoryBot.create(:value_definition, value_type: 'boolean', security_guide: item.security_guide)
        end
  ...
  context '/policies/:id/tailorings/:id/tailoring_file' do
    RSpec.shared_examples 'tailoring_file' do
      ...
      context 'with default, no unselected, rules and values' do
        let(:item) do
          FactoryBot.create(:tailoring, policy: parent, os_minor_version: 8)
        end
      ...
      context 'with no tailored rules, but tailored values' do
        let(:item) do
          FactoryBot.create(:tailoring, :without_rules, :with_tailored_values, policy: parent, os_minor_version: 8)
        end
      ...
      context 'with randomly distributed rules' do
        let(:canonical_profile) do
          FactoryBot.create(
            ...
            rule_count: 5,
            value_count: 5,
            security_guide: FactoryBot.create(:security_guide, rule_count: 3, os_major_version: 9),
            os_major_version: 9,
            ref_id_suffix: 'bar',
            supports_minors: [8]
          )
        ...
        let(:extra_params) do
          {
            ...
            policy_id: FactoryBot.create(:policy, account: FactoryBot.create(:account)),
            id: item.id
          }
        end

        let(:item) { FactoryBot.create(:tailoring, policy: parent, os_minor_version: 8) }
