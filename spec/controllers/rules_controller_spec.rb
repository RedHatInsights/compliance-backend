...
describe RulesController do
  ...
  context '/security_guides/:id/rules' do
    describe 'GET index' do
      let(:parent) { FactoryBot.create(:security_guide) }
      let(:rule_group) { FactoryBot.create(:rule_group, security_guide: parent) }
    ...
    describe 'GET show' do
      let(:item) { FactoryBot.create(:rule) }
      let(:parent) { item.security_guide }
      let(:extra_params) { { security_guide_id: parent.id, id: item.id } }
      let(:notfound_params) { extra_params.merge(security_guide_id: FactoryBot.create(:security_guide).id) }
  ...
  context '/security_guides/:id/profiles/:id/rules' do
    ...
    describe 'GET index' do
      let(:parent) { FactoryBot.create(:profile) }
      let(:rule_group) { FactoryBot.create(:rule_group, security_guide: parent.security_guide) }
      let(:extra_params) do
  ...
  context '/policies/:id/tailorings/:id/rules' do
    let(:parent) do
      FactoryBot.create(
        :tailoring,
        policy: FactoryBot.create(:policy, account: current_user.account, supports_minors: [9]),
        os_minor_version: 9
      )
    end

    describe 'GET index' do
      let(:rule_group) { FactoryBot.create(:rule_group, security_guide: parent.profile.security_guide) }
      let(:extra_params) do
        {
    ...
    describe 'POST create' do
      ...
      let(:items) { FactoryBot.create_list(:rule, 4, security_guide: parent.security_guide) }
      ...
      %i[id ref_id].each do |key|
        context "access via #{key}" do
          ...
          context 'already assigned rule' do
            let(:item) { FactoryBot.create(:rule, security_guide: parent.security_guide, tailoring_id: parent.id) }
    ...
    describe 'PATCH update' do
      let(:item) do
        FactoryBot.create(:rule, security_guide: parent.profile.security_guide)
      end
      ...
      context 'rule already linked to the tailoring' do
        let(:item) do
          FactoryBot.create(:rule, security_guide: parent.profile.security_guide, tailoring_id: parent.id)
        end
      ...
      context 'tailoring belongs to another account' do
        let(:parent) do
          FactoryBot.create(
            :tailoring,
            policy: FactoryBot.create(:policy, supports_minors: [9]),
            os_minor_version: 9
          )
        end
