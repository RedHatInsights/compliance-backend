...
module Kafka
  ...
  class ReportParser
    ...
    def report_validation_error(exception)
      msg = "[#{org_id}] Report validation failed: #{exception.class.to_s.demodulize}"
      @logger.error msg
      @logger.audit_fail msg
      ...
      ReportUploadFailed.deliver(
        system: V2::System.find_by(id: id, org_id: org_id),
        request_id: request_id,
        error: "Report validation failed: #{exception.class.to_s.demodulize}",
        org_id: org_id
      )
      notify_payload_tracker(:error, "#{msg}: #{exception.message}")
      produce_validation_message('failure')
    end
