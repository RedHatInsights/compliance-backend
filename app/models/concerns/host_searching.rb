  ...
  module ClassMethods
    ...
    def compliant_host_ids(value)
      ...
      TestResult.latest
                ...
                .group(:host_id, 'v1_policies.id', 'v1_policies.compliance_threshold')
                .select(:host_id).having('AVG(v1_test_results.score) <= 100')
                .having("
                  AVG(v1_test_results.score)
                  #{to_b(value) ? '>=' : '<'}
                  v1_policies.compliance_threshold
                ")
    end
